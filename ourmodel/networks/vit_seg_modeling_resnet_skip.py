...
class ChannelAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        self.pool = nn.AvgPool2d(kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
...
class TransResNetV2(nn.Module):
    ...
    config.trans.depths = [2, 2, 6, 2] '''
        ...
        for i_layer in range(self.num_layers):  # i_layer 0,1,2,3
            ...
            downsample = PatchMerging(input_resolution=input_resolution, dim=dim, norm_layer=norm_layer)
            ...
            Conv3x3 = BasicConv(in_planes=dim, out_planes=outchannel[i_layer], kernel_size=1, stride=1,
        ...
        self.layers=Biformerblock(img_size=config.data.img_size,n_win=config.trans.window_size).stages
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            # print('q',trans_x.shape)
            trans_x = layer(trans_x)
            # print('h',trans_x.shape)
            # q torch.Size([8, 4096, 96])
            # h torch.Size([8, 4096, 96])
            # q torch.Size([8, 1024, 192])
            # h torch.Size([8, 1024, 192])
            # q torch.Size([8, 256, 384])
            # h torch.Size([8, 256, 384])
            # q torch.Size([8, 64, 768])
            # h torch.Size([8, 64, 768]
