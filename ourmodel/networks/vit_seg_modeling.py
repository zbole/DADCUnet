...
class DecoderCup(nn.Module):
    ...
    def forward(self, x1, features=None):
...
class up(nn.Module):
    def __init__(self, mode='bilinear', size=False):
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=256, num_classes=6, zero_head=False):
        ...
        self.config = config
        self.up2 = up( size=(img_size, img_size))
        
        self.up3 = up(size=(500, 500))
    ...
    def load(self):
        ...
        t1 = r"\R50-ViT-B_16_224\efficientnet-b4_imagenet_R50-ViT-B_16_224 copy.pt"

        # self.transformer.to(device)
        # self.decoder.to(device)

        try:
            pretrained_dict = torch.load(t1, map_location=device,weights_only =True)
            # 加载 self.transformer 的权重
            model_dict_transformer = self.transformer.state_dict()
            pretrained_dict_transformer = {k: v for k, v in pretrained_dict.items() if k in model_dict_transformer}
            model_dict_transformer.update(pretrained_dict_transformer)
            self.transformer.load_state_dict(model_dict_transformer, strict=False)
            # print("self.transformer 预训练权重加载成功！")

            # 加载 self.decoder 的权重
            model_dict_decoder = self.decoder.state_dict()
            pretrained_dict_decoder = {k: v for k, v in pretrained_dict.items() if k in model_dict_decoder}
            model_dict_decoder.update(pretrained_dict_decoder)
            self.decoder.load_state_dict(model_dict_decoder, strict=False)
            # print("self.decoder 预训练权重加载成功！")
