...
def linear_stretch(image, percent):
    """
    对图像进行线性拉伸
    :param image: 输入图像，形状为 (C, H, W)，值为小数
    :param percent: 拉伸百分比
    :return: 拉伸后的图像，形状为 (C, H, W)，值为0-1之间的小数
    """
    # 确保输入是小数并初始化拉伸后的图像
...
def calculate_acc(predictions, masks):
    ...
    # plt.imshow(mask_ones.cpu().numpy()[0][0])
    # plt.show()
    # 计算预测为1的像素中正确预测的个数
    # num_correct = torch.eq(pred_ones, mask_ones).sum().float().item()
    ...
    with torch.no_grad():
        ...
        if outfile is not None :
            # for i in range(2):
                ...
                img=linear_stretch(image.numpy(), 2)
...
def calc_auc(pred_arr, gt_arr, mask_arr=None):
    ...
    if len(np.unique(gt_vec)) >= 2:
        ...
        print("Only one class present, skipping AUC calculation.")
        roc_auc = 0
    return roc_auc*100
